...
const { token } = require("./herşey/ayar/config.json")

const db = require("croxydb")

require("./herşey/ayar/app.js")(client)
client.login(token)
const Emojis = require("./herşey/gerekliler/emoji.json")

const Colors = require("./herşey/gerekliler/renk.json")

const Logs = require("./herşey/gerekliler/log.json")
...
client.on('guildCreate', sunucu => {
  const Eklendim = new EmbedBuilder()
     ...
     .addFields(
       {
         name: `${Emojis.Pusula} Sunucu`,
         value: `> ${sunucu} \`(${sunucu.id})\``
       },
       {
         name: `${Emojis.Taç} Sunucu sahibi`,
         value: `> ${client.users.cache.get(sunucu.ownerId).tag} \`(${sunucu.ownerId})\``
       },
       {
         name: `${Emojis.Sunucu} Toplam sunucu`,
...
process.on("unhandledRejection", hata => { 
  const Hata = new EmbedBuilder()
    .setColor(Colors.Red)
    .setDescription(`> botda bir hata var hata: \`${hata}\``) 
    .setFooter({text: client.user.username, iconURL: client.user.avatarURL()}) 
    .setTimestamp()
  ...
  Linkler.forEach(link => {
    try {
      ...
      const UptimeHata = new EmbedBuilder()
       .setColor(Colors.Red)
       .setDescription(`> link uptime edilirken bir hata meydana geldi hata:  \`${e}\``) 
       .setFooter({text: client.user.username, iconURL: client.user.avatarURL()}) 
       .setTimestamp()
...
const LinkEklemeSistemi = new ActionRowBuilder().addComponents(LinkEkleFormu)
  LinkEklemeFormu.addComponents(LinkEklemeSistemi)
    ...
    const FazlaLink = new EmbedBuilder()
      ...
      .setDescription(`${Emojis.Çarpı} link ekleme limitin doldu en az **50** link ekleyebilirsin`)
      .setFooter({text: client.user.username, iconURL: client.user.avatarURL()}) 
      .setTimestamp()
    ...
    const PreYok = new EmbedBuilder()
      .setColor(Colors.Red)
      .setAuthor({name: interaction.user.username, iconURL: interaction.user.avatarURL()})
      .setDescription(`${Emojis.Çarpı} Premium üyeliğin bulunmadığı için en fazla ${Limit} link ekleyebilirsin kanki`)
      .setFooter({text: client.user.username, iconURL: client.user.avatarURL()}) 
      .setTimestamp()
    ...
    const LinkVar = new EmbedBuilder()
      .setColor(Colors.Red)
      .setAuthor({name: interaction.user.username, iconURL: interaction.user.avatarURL()}) 
      .setDescription(`${Emojis.Çarpı} Belirtilen link zaten sistemde bulunuyor ki`)
      .setFooter({text: client.user.username, iconURL: client.user.avatarURL()}) 
      .setTimestamp()
    ...
    const UptimeLog = new EmbedBuilder()
      ...
      .addFields(
        {
          name: `${Emojis.Kullanıcı} Ekleyen Kullanıcı`, 
...
const LinkYok = new EmbedBuilder()
  .setColor(Colors.Red)
  .setAuthor({name: interaction.user.username, iconURL: interaction.user.avatarURL()}) 
  .setDescription(`${Emojis.Çarpı} Silmeye çalıştığın link sistemde bulunmuyor ki zaten`)
  .setFooter({text: client.user.username, iconURL: client.user.avatarURL()}) 
  .setTimestamp()
...
const EskiLink = new TextInputBuilder()
  ...
  .setLabel('Eski Glitch veya replit proje link girin')
  .setStyle(TextInputStyle.Paragraph)
  .setMinLength(20)
  .setMaxLength(100)
  .setPlaceholder('https://proje-linki.glitch.me')
...
const YeniLink = new TextInputBuilder()
  ...
  .setLabel('Yeni Glitch veya replit proje link girin')
  .setStyle(TextInputStyle.Paragraph)
  .setMinLength(20)
  .setMaxLength(100)
  .setPlaceholder('https://proje-linki.glitch.me')
...
DüzenlemeFormu.addComponents(YeniSistem)
  ...
  const LinkVar = new EmbedBuilder()
    .setColor(Colors.Red)
    .setAuthor({name: interaction.user.username, iconURL: interaction.user.avatarURL()}) 
    .setDescription(`${Emojis.Çarpı} Düzenlerken eklemeye çalıştığın link zaten sistemde bulunuyor ki knk`)
    .setFooter({text: client.user.username, iconURL: client.user.avatarURL()}) 
    .setTimestamp()
  return await interaction.followUp({embeds: [LinkVar], ephemeral: true})
  }
  ...
  if(!link.startsWith("https://") && !link.startsWith("http://")) {
    ...
    const SonuHatalı = new EmbedBuilder()
      .setColor(Colors.Red)
      .setAuthor({name: interaction.user.username, iconURL: interaction.user.avatarURL()}) 
      .setDescription(`${Emojis.Çarpı} Sistemimiz sadece glitch desteklemektedir, linkin sonunda \`.glitch.me\` olduğundan emin ol`)
      .setFooter({text: client.user.username, iconURL: client.user.avatarURL()}) 
      .setTimestamp()
    return await interaction.followUp({embeds: [SonuHatalı], ephemeral: true})
    }
...
//bu altyapı venom uptimeye ayitdir.