...
async execute(client, interaction) {
  ...
  const KodMiktarı = new TextInputBuilder()
    ...
    .setLabel('Kodun vereceği link limit miktarı yaz')
    .setStyle(TextInputStyle.Short)
    .setPlaceholder('5')
    .setRequired(true)
  ...
  if(!Kod) {
    ...
    const Var = new EmbedBuilder()
      .setColor(Colors.Red)
      .setAuthor({name: interaction.user.username, iconURL: interaction.user.avatarURL()}) 
      .setDescription(`${Emojis.Çarpı} Bu promosyon kodu zaten sistemde bulunuyor. ki kral`)
      .setFooter({text: client.user.username, iconURL: client.user.avatarURL()}) 
      .setTimestamp()
  ...
  const KodSilFormu = new TextInputBuilder()
    .setCustomId('kod')
    .setLabel('Silinecek promosyon kodunu yaz!')
  ...
  if(!Kod) {
    const Yok = new EmbedBuilder()
      .setColor(Colors.Red)
      .setAuthor({name: interaction.user.username, iconURL: interaction.user.avatarURL()}) 
      .setDescription(`${Emojis.Çarpı} Bu promosyon kodu sistemde bulunmuyor ki`)
      .setFooter({text: client.user.username, iconURL: client.user.avatarURL()}) 
      .setTimestamp()
  ...
  let Liste
  if(!Kodlar) {
    Liste = `${Emojis.Çarpı} Promosyon kodu bulunmuyo kral.`
  } else {
  ...
  const KodListe = new EmbedBuilder()
    ...
    .setAuthor({name: interaction.user.username, iconURL: interaction.user.avatarURL()}) 
    .setDescription(`VENOM PROMOSYON KODU LİSTESİ`)
    .addFields(
      {
  ...
  if(!Kod) {
    const Yok = new EmbedBuilder()
      .setColor(Colors.Red)
      .setAuthor({name: interaction.user.username, iconURL: interaction.user.avatarURL()}) 
      .setDescription(`${Emojis.Çarpı} Sistemde böyle bir promosyon kodu bulunmuyor kk kodu sallama`)
      .setFooter({text: client.user.username, iconURL: client.user.avatarURL()}) 
      .setTimestamp()
    await interaction.followUp({embeds: [Yok]})
  } else { 
      ...
      if(!KodKullanıldı) {
        ...
        const Kullanmışsın = new EmbedBuilder()
          .setColor(Colors.Red)
          .setAuthor({name: interaction.user.username, iconURL: interaction.user.avatarURL()}) 
          .setDescription(`${Emojis.Çarpı} Bu promosyon kodunu sen zaten kullanmışsın sansın yok ki beee!`)
          .setFooter({text: client.user.username, iconURL: client.user.avatarURL()}) 
          .setTimestamp()
        ...
        const Dolu = new EmbedBuilder()
          .setColor(Colors.Red)
          .setAuthor({name: interaction.user.username, iconURL: interaction.user.avatarURL()}) 
          .setDescription(`${Emojis.Çarpı} Bu promosyon kodunun kullanım limiti dolmuş. Kullanım limiti: \`${ToplamKullanım}\` di yaaa!`)
          .setFooter({text: client.user.username, iconURL: client.user.avatarURL()}) 
          .setTimestamp()
