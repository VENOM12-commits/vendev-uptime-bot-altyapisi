...
module.exports = {
  ...
  dbl: false, 
  name: ['link'],  
  description: 'venom link ekleme sistemi.',
  options: [
    { 
      name: "ekle",
...
const LinkEkleFormu = new TextInputBuilder()
  ...
  .setLabel('Glitch veya replit link girin')
...
const LinkEklemeSistemi = new ActionRowBuilder().addComponents(LinkEkleFormu)
  LinkEklemeFormu.addComponents(LinkEklemeSistemi)
    ...
    const FazlaLink = new EmbedBuilder()
      ...
      .setDescription(`${Emojis.Çarpı} Bir kullanıcı tarafından en az **30** link eklenebilir kanki premiun varsa **50** link`)
    ...
    const PreYok = new EmbedBuilder()
      .setColor(Colors.Red)
      .setAuthor({name: interaction.user.username, iconURL: interaction.user.avatarURL()}) 
      .setDescription(`${Emojis.Çarpı} Premium üyeliğin bulunmadığı için en fazla ${Limit} link ekleyebilirsin oleyyyy`)
      .setFooter({text: client.user.username, iconURL: client.user.avatarURL()}) 
      .setTimestamp()
    ...
    const LinkVar = new EmbedBuilder()
      .setColor(Colors.Red)
      .setAuthor({name: interaction.user.username, iconURL: interaction.user.avatarURL()}) 
      .setDescription(`${Emojis.Çarpı} Belirtilen link zaten sistemde bulunuyor kk başka link dene!`)
      .setFooter({text: client.user.username, iconURL: client.user.avatarURL()}) 
      .setTimestamp()
    ...
    const BaşıHatalı = new EmbedBuilder()
      .setColor(Colors.Red)
      .setAuthor({name: interaction.user.username, iconURL: interaction.user.avatarURL()}) 
      .setDescription(`${Emojis.Çarpı} Eklemek istediğin linkin \`https://\` başladığından emin ol`)
      .setFooter({text: client.user.username, iconURL: client.user.avatarURL()}) 
      .setTimestamp()
...
const LinkYok = new EmbedBuilder()
  .setColor(Colors.Red)
  .setAuthor({name: interaction.user.username, iconURL: interaction.user.avatarURL()}) 
  .setDescription(`${Emojis.Çarpı} Silmeye çalıştığın link sistemde bulunmuyor kral`)
  .setFooter({text: client.user.username, iconURL: client.user.avatarURL()}) 
  .setTimestamp()
...
const PremiumÜye = db.fetch(`PremiumÜye_${interaction.user.id}`)
  ...
  const Silindi = new EmbedBuilder()
    .setColor(Colors.Green)
    .setAuthor({name: interaction.user.username, iconURL: interaction.user.avatarURL()}) 
    .setDescription(`${Emojis.Tik} Belirtilen link sistemden sildim`)
    .setFooter({text: client.user.username, iconURL: client.user.avatarURL()}) 
    .setTimestamp()
...
const EskiLink = new TextInputBuilder()
  ...
  .setLabel('Eski proje adresinizi girin!')
  .setStyle(TextInputStyle.Paragraph)
  .setMinLength(20)
  .setMaxLength(100)
  .setPlaceholder('https://proje-linki.glitch.me')
...
const YeniLink = new TextInputBuilder()
  ...
  .setLabel('Yeni proje adresinizi girin!')
  .setStyle(TextInputStyle.Paragraph)
  .setMinLength(20)
  .setMaxLength(100)
  .setPlaceholder('https://proje-linki.glitch.me')
...
DüzenlemeFormu.addComponents(YeniSistem)
  ...
  const LinkVar = new EmbedBuilder()
    .setColor(Colors.Red)
    .setAuthor({name: interaction.user.username, iconURL: interaction.user.avatarURL()}) 
    .setDescription(`${Emojis.Çarpı} Düzenlerken eklemeye çalıştığın link zaten sistemde bulunuyor kral başka dene`)
    .setFooter({text: client.user.username, iconURL: client.user.avatarURL()}) 
    .setTimestamp()
  return await interaction.followUp({embeds: [LinkVar]})
  }
  ...
  const LinkYok = new EmbedBuilder()
    .setColor(Colors.Red)
    .setAuthor({name: interaction.user.username, iconURL: interaction.user.avatarURL()}) 
    .setDescription(`${Emojis.Çarpı} Düzenlerken silmeye çalıştığın link zaten sistemde bulunmuyor ki`)
    .setFooter({text: client.user.username, iconURL: client.user.avatarURL()}) 
    .setTimestamp()
  ...
  if(!link.startsWith("https://") && !link.startsWith("http://")) {
    const BaşıHatalı = new EmbedBuilder()
      .setColor(Colors.Red)
      .setAuthor({name: interaction.user.username, iconURL: interaction.user.avatarURL()}) 
      .setDescription(`${Emojis.Çarpı} Düzenlerken eklemek istediğin linkin \`https://\` başladığından emin ol.`)
      .setFooter({text: client.user.username, iconURL: client.user.avatarURL()}) 
      .setTimestamp()
    return await interaction.followUp({embeds: [BaşıHatalı]})
    }
    ...
    const SonuHatalı = new EmbedBuilder()
      .setColor(Colors.Red)
      .setAuthor({name: interaction.user.username, iconURL: interaction.user.avatarURL()}) 
      .setDescription(`${Emojis.Çarpı} Sistemimiz sadece glitch ve replit desteklemektedir, linkin sonunda \`.glitch.me\` veya \`.repl.co\`  olduğundan emin ol`)
      .setFooter({text: client.user.username, iconURL: client.user.avatarURL()}) 
      .setTimestamp()
    return await interaction.followUp({embeds: [SonuHatalı]})
    }
  ...
  if(interaction.options.getSubcommand() === 'admin-liste') {
    ...
    const Sahip = new EmbedBuilder()
      ...
      .setFooter({text: client.user.username, iconURL: client.user.avatarURL()}) 
      .setTimestamp()
      if(config.sahipID) return await interaction.reply({embeds: [Sahip]})
...
const SilFormu = new TextInputBuilder()
  .setCustomId('sil')
  .setLabel('Glitch veya Replit linkinizi girin!')
  .setStyle(TextInputStyle.Paragraph)
  .setMinLength(20)
  .setMaxLength(100)
  .setPlaceholder('https://proje-linki.glitch.me')
...
const Sebep = new TextInputBuilder()
  .setCustomId('sebep')
  .setLabel('Projeyi silme sebebini belirtin lütfen.')
  .setStyle(TextInputStyle.Paragraph)
...
await interaction.deferReply()
  ...
  const ProjeYok = new EmbedBuilder()
    .setColor(Colors.Red)
    .setAuthor({name: interaction.user.username, iconURL: interaction.user.avatarURL()}) 
    .setDescription(`${Emojis.Çarpı} Bu kullanıcının böyle bir linki bulunmuyor kankim.`)
    .setFooter({text: client.user.username, iconURL: client.user.avatarURL()}) 
    .setTimestamp()
