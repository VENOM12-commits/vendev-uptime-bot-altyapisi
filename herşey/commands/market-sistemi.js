...
const { supportServer } = require("../ayar/config.json")
...
module.exports = {
  ...
  dbl: false, 
  name: ['market'],  
  description: 'venom market sistemi',
    ...
    .addFields(
      {
        ...
        value: `+1 link ekleme hakkı - **30 ${paraBirimi}**`
      },
      {
        name: `${Emojis.Elmas} Premium üyelik`,
        value: `1 aylık premium üyeliği - **300 ${paraBirimi}**`
      },
      {
        name: `${Emojis.Para} VenCoin(${paraBirimi})`,
        value: `${paraBirimi} almak için destek sunucusunda sahibim ile iletişime geçin kral`
      })
    ...
    if(interaction.customId === `linklimital_${interaction.user.id}`) {
        ...
        await interaction.awaitModalSubmit({ filter: (interaction) => interaction.customId === `limitalmaform`, time: 60000 }).then(async (interaction) => {
          ...
          const ParaYok = new EmbedBuilder()
            .setColor(Colors.Red)
            .setAuthor({name: interaction.user.username, iconURL: interaction.user.avatarURL()}) 
            .setDescription(`${Emojis.Çarpı} Almak istediğin **${Limit}** link limit hakkı için **${Limit*30} ${paraBirimi}** gerekli kral`)
            .setFooter({text: client.user.username, iconURL: client.user.avatarURL()}) 
            .setTimestamp()
          ...
          const Alındı = new EmbedBuilder()
            ...
            .setAuthor({name: interaction.user.username, iconURL: interaction.user.avatarURL()}) 
            .setDescription(`${Emojis.Tik} **${Limit}** link limit hakkı **${Limit*30} ${paraBirimi}** karşılığında satın aldı kral bee`)
            .setFooter({text: client.user.username, iconURL: client.user.avatarURL()}) 
            .setTimestamp()
      ...
      if(interaction.customId === `premiumal_${interaction.user.id}`) {
        ...
        if(Premium) {
           const PreVar = new EmbedBuilder()
            .setColor(Colors.Red)
            .setAuthor({name: interaction.user.username, iconURL: interaction.user.avatarURL()}) 
            .setDescription(`${Emojis.Çarpı} Zaten aktif olarak premium üyeliğin bulunuyor ki ne gerek var!`)
            .setFooter({text: client.user.username, iconURL: client.user.avatarURL()}) 
            .setTimestamp()
          ...
          const ParaYok = new EmbedBuilder()
            .setColor(Colors.Red)
            .setAuthor({name: interaction.user.username, iconURL: interaction.user.avatarURL()}) 
            .setDescription(`${Emojis.Çarpı} Premium almak için **300 ${paraBirimi}** gerekli`)
            .setFooter({text: client.user.username, iconURL: client.user.avatarURL()}) 
            .setTimestamp()
